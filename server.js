    ...
    ws.on('message', (message) => {
        try {
            ...
            if (data.type === 'join') {
                ...
                // 添加玩家
                const paddlePositions = getPaddlePositions(game.players.length + 1);
                const playerIndex = game.players.length;
                const player = {
                    id: data.playerId,
                    color: data.color,
                    x: paddlePositions[playerIndex],
...
function gameLoop() {
    ...
    for (const brick of game.bricks) {
        ...
        if (rectIntersect(ballRect, brickRect)) {
            brick.alive = false;
            // 找到最后击打砖块的玩家加分
            const lastHitter = game.players[Math.floor(Math.random() * game.players.length)];
            if (lastHitter) lastHitter.score += CONFIG.POINTS_PER_BRICK;
